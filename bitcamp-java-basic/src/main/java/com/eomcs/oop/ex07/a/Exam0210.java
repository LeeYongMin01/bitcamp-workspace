...
class Score2 {
  ...
  // sum과 aver의 값을 직접 변경하지는 못하고
  // 메소드를 이용하여 가능
  // => 보통 아래와 같이 필드의 값을 조회하는 용도로 사용하는 메서드의 경우
...
public class Exam0210{
  public static void main(String[] args) {
    ...
    // 계산을 한 후에 임의적으로 합계나 평균을 변경하려면?
    // => sum과 aver 필드는 private 접근만 허용한다.
    // => 즉, 클래스의 멤버만 접근할 수 있고 클래스 외부에서는 접근하지 못한다.
    // => 그래서 임의로 접근하여 값을 변경할 수 없다.
