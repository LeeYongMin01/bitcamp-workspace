...
// 인스턴스 변수(=인스턴스 필드) = non-static variable
...
public class Exam0110 {
  ...
  class A {
    // 인스턴스 변수
    // => new 명령을 통해 생성된다.
    // => new 명령을 실행하기 전까지 인스턴스 변수는 존재하지 않음
    // => Heap 영역에 생성된다.
    // => static이 붙지 않는다.
    int v1; // 4바이트 int 값을 저장할 메모리를 만들라는 명령
    boolean v2; // 논리값을 저장할 메모리를 만들라는 명령
  }

  // A 클래스에 대해 new 명령을 사용하기 전에는
  // v1, v2 메모리는 존재하지 않고 단지 설계도에 불과하다
