...
public class Exam0580 {
  public static void main(String[] args) {
    ...
    // - 특정 조건이 되면 가비지 수집기(garbage collector)에 의해 메모리가 해제된다.
    // 메모리 해제? 다른 용도로 사용할 수 있도록 표시 한다는 뜻
    
    // ## 가비지 수집 조건 = 가비지 컬렉터가 동작할 때
    //- 메모리가 부족할 때
    // - 운영체제로부터 메모리는 추가로 받기 전에 먼저 기존에 사용하던 메모리에서
    // 가비지를 제거한다.
    // -cpu가 한가할 때 
    ...
    // - c 언어 => free(메모리주소);
    // -c++ 언어 => delete 객체주소;
    // - 요즘 언어의 트렌드는 사용하지 않는 메모리를 개발자가 직접 해제하는 것이 아니라
    //VM이 해제하는 것이다.
    // 예) JavaScript, C#, Python, PHP, Go, Java 등
    // 요즘언어의 트렌드는 VM으로 실행하는 것이다.
    // 왜? 직접 기계어로 전환되면 메모리 관리는 자동으로 수행할 수 없다.
    // # 가비지 컬렉터는 강제로 실행하는 방법?
    // 없다
    // 단 원래 계획보다 가능한 빨리 실행하라고 독촉하는 방법은 있다.
    // System.gc() 메서드 호출
    // - 그런데 바로 실행할지 나중에 실행할지 그 시점을 보장하지는 않음
